...
public class BlackJack {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < deck.length; i++) {
			System.out.println("카드위치" + deck[i] + ":" + cardShape[i / 13] + cardNum[i % 13]);
		}
		// 카드 교환
//		int cardIndex = 51;
//		int tempCard=deck[0];
...
//		}
		...
		for (int i = 0; i < deck.length; i++) {
			System.out.println("카드위치" + i + ": " + "카드번호 " + cardShape[deck[i] / 13] + cardNum[deck[i] % 13]);
		}
...
//		}System.out.print("입니다. ");
		...
		while (isP1PlayFlag || isP2PlayFlag) {
			...
			p1Sum=0;
			for (int i = 0; i < p1DeckIndex; i++) {
				if (p1Deck[i] % 13 < 1) {// p1Deck 카드가 A이면 11을 더한다.
					p1Sum = p1Sum + 11;
				} else if (p1Deck[i] % 13 < 10) {// p1Deck[i] 2~10이면 2~10을 더한다.
			...
			p2Sum=0;
			for (int i = 0; i < p2DeckIndex; i++) {
				if (p2Deck[i] % 13 < 1) {// p2Deck 카드가 A이면 11을 더한다.
					p2Sum = p2Sum + 11;
				} else if (p2Deck[i] % 13 < 10) {// p2Deck[i] 2~10이면 2~10을 더한다.
